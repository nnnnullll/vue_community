  ...
  <div>
    ...
    <div v-if="tableData != null" class="container">
      ...
      >
        ...
        <el-table-column sortable prop="company" label="物业公司编号"></el-table-column>
        <el-table-column prop="number" v-if="ifshow">
          <template slot-scope="{ row }">
            <el-link v-if="row.company==usercompany"  type="primary" @click="update(row.number, 1)">解除合作</el-link>
            <el-link v-if="row.company==null" type="primary" @click="update(row.number, 2)">建立合作</el-link>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    getDataCommunityByCompany (companynumber) {
      axios
        .post('/getcommunity?type=3&number=1&company=0')
        .then(res => {
